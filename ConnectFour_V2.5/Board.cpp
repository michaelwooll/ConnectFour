...
void Board::multiplayer(){
    ...
    //Creating or loading user 1
    cout << "Enter Player 1's name";
    cin >> p1User;
    p1 = Player(p1User);
    
    //Creating filepath for user1
    fileName = "./Players/";
    fileName += p1User;
    fileName += ".bin"; 
    
    //Checking if file can be opened, if not create new user
    in.open(fileName,ios::in | ios::binary);
    if(!in.is_open()){
        cout << "Player not found in files. Creating new user..." << endl;
        Player createdPlayer(p1);
        p1 = createdPlayer;
        p1.save();
    }
    else{
        cout << "User found. Loading in player file." << endl;
        Player loadedPlayer(p1);
        loadedPlayer.load();
        p1 = loadedPlayer;
    }
    
    // creating or laoding second user
    cout << "Enter Player 2's name";
    cin >> p2User;
    p2 = Player(p2User);
    
    //Creating file path for user2
    fileName2 = "./Players/";
    fileName2 += p2User;
    fileName2 += ".bin"; 
    
    //Checking if file can be opened, if not create a new user
    in.open(fileName2,ios::in | ios::binary);
    if(!in.is_open()){
        cout << "Player not found in files. Creating new user..." << endl;
        Player createdPlayer(p2);
        p2 = createdPlayer;
        p2.save();
    }
    else{
        cout << "User found. Loading in player file." << endl;
        Player loadedPlayer(p2);
        loadedPlayer.load();
        p2 = loadedPlayer;
    }
    ...
    // Directions
    cout << p1.getName() << " is X's and goes first." << endl;
    cout << p2.getName() << " is O's" << endl << endl;
    ...
    //Save users and print out records
    p1.save();
    p2.save();
    p1.displayRec() ;
   cout << endl;
